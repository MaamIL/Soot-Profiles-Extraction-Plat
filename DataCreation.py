...
class FlameDataset(Dataset):
    ...
    def _getT_(self, soot_mat):
        ...
        T = soot_mat["T"]
        T[T < 1000.0] = 300.0       
        #Temperature padding needs to be with values of 300 (kalvin as the minimum value is 300)
        T  = np.pad(T,((0, self.config.output_shape[0] - T.shape[0]), (0, self.config.output_shape[1] - T.shape[1])), mode='constant', constant_values=300)   # shape: (202, 92)
        if(self.config.isNorm):
            # Normalize
    ...
    def __getitem__(self, idx):        
        ...
        image_array[image_array < self.config.setImgValZero] = 0.0
        #normelize
        # image_array = image_array/4095.0
        image_array = image_array/max(image_array.max(), 1e-6)  # Avoid division by zero
        #padding
